...
def main(llm, tokenizer, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            # for text in outputs:
            #     if "</think>" in text:
            #         # 找到最后一个 </think> 并截取后面部分
            #         end_index = text.rindex("</think>") + len("</think>")
            #         content_after_think = text[end_index:].strip()
            #     else:
            #         # 如果没找到 </think>，保留原始输出
            #         content_after_think = text.strip()

            #     processed_outputs.append(content_after_think)
