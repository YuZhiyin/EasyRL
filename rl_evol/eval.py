...
from EasyRL.RL.RLEvol.utils import *
from EasyRL.RL.RLEvol.config import *
...
class Evaluator:
    ...
    def __init__(
        ...
        self.output_path = f'./save/tmp/{task}_{self.model}_{self.timestamp}.json'
        self.gptreq = None
    ...
    def _process_responses(self, res_list: List[Dict], extract_fn: Callable):
        ...
        for i, (sample, result) in enumerate(zip(self.samples, res_list)):
            ...
            # if i==0:
                # print("\n--- [Debug] First Sample Processed ---")
                
            # 打印 Pred (模型的原始完整回答)
            # print(f"Pred (Raw Response): {self.samples[i].get('Pred', 'N/A')}")
                
            #     # 打印 PredAnswer (提取出的答案)
            # print(f"PredAnswer (Extracted): {self.samples[i].get('PredAnswer', 'N/A')}")
            # print("-------------------------------------\n")
            
    def run_inference(self, format_fn: Callable, extract_fn: Callable, flag: bool) -> float:
        ...
        # print("acc score: ",self.calculate_accuracy(TASK_CONFIG[self.task]['check_fn']))
        
        # return self.calculate_accuracy(TASK_CONFIG[self.task]['check_fn'])
        # print(self.samples[0])
        
        # print("acc score: ",self.calculate_accuracy())
    ...
    def calculate_accuracy(self) -> float:
        """Calculate accuracy of predictions"""
        scores = [
            1.0 if math_equal(s['PredAnswer'], s["answer"]) else 0.0
            for s in self.samples
        ]
        return np.mean(scores)
    """
    'PredAnswer': '\\cos^{-1}(\\frac{13}{\\sqrt{226}})'
    'answer': 31.0
    """
