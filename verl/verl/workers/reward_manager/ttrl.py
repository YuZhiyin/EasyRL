...
class TTRLRewardManager:
            ...
            for prompt_i in range(prompt_num):
                ...
                for i in range(self.n_votes_per_prompt):
                    ...
                    if task is None:
                        ...
                        if task != self._data_source_to_task(data_source):
                            raise NotImplementedError(f"Non consistent task {task} and {self._data_source_to_task(data_source)} for TTRLRewardManager")
                    if "entropys" in data_item.batch:
                        entropy = data_item.batch["entropys"]  # 假设这是token级别的entropy
                        group_entropys.append(entropy)
                    else:
                        group_entropys.append(None)
                    group_labels.append(ground_truth)
                    group_pred_outputs.append(response_str)
                    group_extra_info.append(extra_info)
                
                rewards, ttrl_metrics = test_time_train_metrics(group_pred_outputs, group_labels, entropys=group_entropys, task=task, extra_info=group_extra_info, reward_type=self.reward_type, entropy_thres=self.entropy_thres)
    ...
    def _compute_eval_reward(self, data: DataProto):
            ...
            for prompt_i in range(prompt_num):
                ...
                _, ttrl_metrics = test_time_train_metrics(group_pred_outputs_ttrl, group_labels_ttrl, entropys=group_entropys, task=task, extra_info=group_extra_info_ttrl, reward_type=self.reward_type, entropy_thres=self.entropy_thres)
                for k, v in ttrl_metrics.items():
                    all_ttrl_metrics[k].append(v)
